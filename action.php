...
if(isset($_POST["getProduct"])){
	...
	$product_query = "SELECT * FROM menu LIMIT $start,$limit";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$pro_id    = $row['menu_id'];
			$pro_cat   = $row['menu_cat'];
			$pro_brand = $row['menu_desc'];
			$pro_title = $row['menu_name'];
			$pro_price = $row['price'];
			$pro_image = $row['menu_image'];
			echo "
				<div class='col-md-4'>
	...
	if($count > 0){
		...
		echo '
				...
				<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
				  ...
				  <input type="hidden" name="business" value="BakeryStore@gmail.com">
				...
				echo   '
				<input type="hidden" name="return" value="http://localhost/BakeryStore/payment_success.php"/>
				<input type="hidden" name="cancel_return" value="http://localhost/BakeryStore/profile.php"/>
